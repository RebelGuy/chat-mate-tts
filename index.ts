...
const CUSTOM_OLLAMA_FILTER = 'tts-filter'

// if the score is lower than this number, it will be rejected by TTS
...
function connectToWebsocket() {
  ...
  socket.addEventListener("message", async event => {
    ...
    if (msg.type === 'event' && msg.data.topic === 'streamerChat') {
      ...
      try {
        const process = Bun.spawn({ cmd: ['ollama', 'run', CUSTOM_OLLAMA_FILTER, `prompt="${text}"`]})
        const output = await new Response(process.stdout).text()
        const numberPart = output.match(new RegExp(/[0-9]+/))
        if (numberPart == null || numberPart[0] == null) {
          throw new Error('Unable to determine score from response ' + output)
        }

        const score = Number(numberPart[0])
        if (score < SPAM_SCORE_THRESHOLD) {
          console.log('Message rejected due to a spam score of', score)
          return
        } else {
        ...
        console.error('Failed to determine legitimacy of the message:', e)
      }
